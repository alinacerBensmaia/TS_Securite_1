...
#Region "*-----    Méthodes Friend    -----*"
    ...
    Friend Function LireFichierCodeAcces() As DataSet
        Dim strNomFichCdAcc As String = XuCuConfiguration.ValeurSysteme("TS1", "TS1\TS1N223\PASSWDFILE")
    ...
    ''' <summary>
    ''' Ecrire le fichier contenant les données des codes d'accès génériques (TS1N213.xml).
    ...
    ''' 	Nom du fichier qui contient les données des codes d'accès génériques TS1N213.xml
    ''' 	Value Type: string
    ''' </param>
    ''' <remarks>
    ''' Historique des modifications: 
    ...
    ''' --------------------------------------------------------------------------------
    ''' </remarks>
    Friend Sub EcrireFichierCodeAcces(ByRef dsCodeAcces As DataSet, ByVal strNomFich As String)
        If Not dsCodeAcces Is Nothing Then
            Using streamContainer As DisposableContainer(Of CryptoStream) = chiffrerFichier(strNomFich)
                dsCodeAcces.WriteXml(streamContainer)
            End Using
        End If
    End Sub
...
#Region "*-----    Méthodes Privées   -----*"
    ...
    ''' <summary>
    ''' Chiffrer le fichier des codes accès.
    ''' </summary>
    ''' <param name="strNomFich">
    ...
    ''' --------------------------------------------------------------------------------
    ''' </remarks>
    Private Function chiffrerFichier(ByVal strNomFich As String) As DisposableContainer(Of CryptoStream)
        Using rijndael As New RijndaelManaged()
            Dim encodeur As ICryptoTransform = rijndael.CreateEncryptor(tsCuVarShared.K, tsCuVarShared.V)
            Dim fichier As New FileStream(strNomFich, FileMode.Create, FileAccess.Write, FileShare.None)
            Dim fichierEncode As New CryptoStream(fichier, encodeur, CryptoStreamMode.Write)

            'puisque l'on ne peut pas disposer des autres objets ici...
            Return New DisposableContainer(Of CryptoStream)(fichierEncode, encodeur, fichier)
        End Using
    End Function

    ''' <summary>
    ''' Déchiffrer le fichier des codes accès.
    ''' </summary>
    ''' <param name="strNomFich">
    ''' 	Nom du fichier XML qui contient les informations des clés symboliques TS1N213.xml
    ''' 	Value Type: string
    ...
    '''                         dans cette méthode.
    ''' 
    ''' 2008-02-20  t206500     Renommer les variables pour qu'elles soient un peu moins 
    '''                         claires à la décompilation (avec reflector)...
    ''' --------------------------------------------------------------------------------
    ''' </remarks>
    Private Function dechiffrerFichier(ByVal strNomFich As String) As DisposableContainer(Of CryptoStream)
        Using rijndael As New RijndaelManaged()
            Dim decodeur As ICryptoTransform = rijndael.CreateDecryptor(tsCuVarShared.K, tsCuVarShared.V)
            Dim fichier As New FileStream(strNomFich, FileMode.Open, FileAccess.Read, FileShare.Read)
            Dim fichierDecode As New CryptoStream(fichier, decodeur, CryptoStreamMode.Read)

            'puisque l'on ne peut pas disposer des autres objets ici...
            Return New DisposableContainer(Of CryptoStream)(fichierDecode, decodeur, fichier)
        End Using
    End Function
        ...
        Using donneesTemporaires As New DataSet()
            ...
            If (cacheVide) OrElse (nomFichierDifferent) OrElse (fichierEteModifie) Then
                ...
                Do While quantiteEssais < MAX_ESSAIS
                    Try
                        Using streamContainer As DisposableContainer(Of CryptoStream) = dechiffrerFichier(strNomFichCdAcc)
                            donneesTemporaires.ReadXml(streamContainer)
